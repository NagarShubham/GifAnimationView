package com.shubham.gif.library
...
class GifAnimationView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyle: Int = R.styleable.GifViewTheme_gifViewStyle
    ...
    private fun setViewAttributes(context: Context, attrs: AttributeSet?, defStyle: Int) {
        ...
        val array = context.obtainStyledAttributes(
            attrs, R.styleable.GifView, defStyle, R.style.Widget_GifView
        )
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        if (movie != null) {
            val movieWidth = movie?.width() ?: 0
            val movieHeight = movie?.height() ?: 0
            /*
			 ...
			 */
            movieScale = 1f / max(scaleH, scaleW)
    ...
    private fun drawMovieFrame(canvas: Canvas) {
        movie?.let {
            it.setTime(currentAnimationTime)
            canvas.save()
            canvas.scale(movieScale, movieScale)
            it.draw(canvas, movieLeft / movieScale, movieTop / movieScale)
            canvas.restore()
        }
    }
